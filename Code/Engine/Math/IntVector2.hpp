...
public:
	//STRUCTORS
	constexpr IntVector2()							: x(),        y()  			{ }
	constexpr IntVector2(int size)					: x(size),    y(size)		{ }
	constexpr IntVector2(int x, int y)				: x(x),       y(y)			{ }
	constexpr IntVector2(const IntVector2& other)	: x(other.x), y(other.y)	{ }

	Vector2 ToVector2() const						{ return Vector2((float)x, (float)y);						}
	String	ToString()	const						{ return std::to_string(x) + "," + std::to_string(y);		}
	float Length()		const						{ return sqrt((float)x * (float)x + (float)y * (float)y);	}
